...
export default function ReadyToServeScreen({ onBack }) {
    ...
    <div className="flex h-full w-full flex-col overflow-hidden bg-[#F5F6F8] text-[#121212]">
      <header className="flex h-16 shrink-0 items-center justify-between bg-[#121212] px-6 text-white">
        ...
        <div className="flex items-center gap-2 font-black uppercase tracking-wider">
          <BellRing className="h-5 w-5 text-emerald-400" /> Ready to Serve / Collect
        </div>
      ...
      <main className="flex-1 overflow-y-auto p-6">
        ...
        {isLoading ? (
          <div className="flex h-full items-center justify-center gap-3 text-gray-500">
            <Loader2 className="h-6 w-6 animate-spin" /> Loading ready orders…
          </div>
        ) : sortedOrders.length === 0 ? (
          <div className="flex h-full flex-col items-center justify-center text-gray-400">
            <BellRing className="mb-3 h-12 w-12" />
            <p className="font-bold text-gray-600">No orders are ready to serve</p>
          </div>
        ) : (
            ...
            {sortedOrders.map((order) => (
              <article key={order.id} className="rounded-2xl border-2 border-emerald-300 bg-white p-5 shadow-sm">
                <div className="flex items-start justify-between gap-3 border-b border-gray-100 pb-3">
                  <div>
                    <p className="text-lg font-black">Order #{order.orderNumber}</p>
                    <p className="mt-1 flex items-center gap-1 text-xs font-bold uppercase text-gray-600">
                      <UtensilsCrossed className="h-3.5 w-3.5" /> {order.diningMode === 'takeaway' ? 'Takeaway Pickup' : `Table ${order.tableNumber || '-'}`}
                    </p>
                ...
                <div className="space-y-3 py-4">
                  ...
                  {order.diningMode === 'takeaway' && order.takeawayPackaging.length > 0 && (
                    <div className="rounded-xl border border-sky-200 bg-sky-50 p-3">
                      <p className="text-[10px] font-black uppercase text-sky-800">Pack with order</p>
                      <p className="mt-1 text-xs font-bold text-sky-950">{order.takeawayPackaging.map((entry) => entry.replaceAll('_', ' ')).join(' · ')}</p>
                    </div>
                  )}
                </div>
                ...
                <div className="flex justify-end border-t border-gray-100 pt-3">
                  ...
                  >
                    {servingId === order.id
                      ? order.diningMode === 'takeaway' ? 'COLLECTING…' : 'SERVING…'
                      : order.diningMode === 'takeaway' ? 'MARK COLLECTED' : 'MARK SERVED'}
                  </button>
                </div>
              </article>
            ))}

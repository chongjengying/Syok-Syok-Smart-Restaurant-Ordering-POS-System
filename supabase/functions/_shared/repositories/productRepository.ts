...
export class ProductRepository {
  ...
  findActive(filters: ProductFilters) {
    let query = this.client
      .from('products')
      .select(
        'id, category_id, product_name, description, unit, sell_price, status, is_available, image_url, categories(id, name)',
        { count: 'exact' },
      )
      .eq('status', true)
      .order('product_name')
  ...
  findActiveById(productId: string) {
    return this.client
      .from('products')
      .select('id, category_id, product_name, description, unit, sell_price, status, is_available, image_url, categories(id, name)')
      .eq('id', productId)
      .eq('status', true)
      .maybeSingle();
  }

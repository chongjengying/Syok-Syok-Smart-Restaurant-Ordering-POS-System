-- Serving completes fulfillment, but does not prove cleaning has started.
-- Keep the table OCCUPIED until front-of-house explicitly starts cleaning.
  ...
  then
    ...
    if not has_other_operational_order then
      perform public.log_table_activity(
        ...
        jsonb_build_object('order_number', new.order_number, 'awaiting_cleaning', true)
      );
    end if;
...
-- Remove the legacy direct table-cleaning write from the private payment
-- worker. The order trigger above now leaves completed tables occupied.
do $$
declare
  definition text;
  old_block constant text := '  if ord.dining_mode = ''dine-in'' and ord.status = ''COMPLETED'' then
    update public.restaurant_tables set status = ''CLEANING'', is_active = true
...
';
begin
  definition := pg_get_functiondef('public.complete_payment(uuid,text,numeric,text,text,text)'::regprocedure);
  if position(old_block in definition) = 0 then raise exception 'EXPECTED_DIRECT_CLEANING_BLOCK_NOT_FOUND'; end if;

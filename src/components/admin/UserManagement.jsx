...
const emptyForm = { name: '', email: '', username: '', phone: '', branchId: '', additionalBranches: [], role: 'WAITER', status: 'ACTIVE', enablePosAccess: true };
...
export default function UserManagement() {
  ...
  const open = (user = null) => {
    ...
    setForm(user ? {
      ...
      phone: user.phone || '', branchId: user.branch_id || '', additionalBranches: [],
    ...
    } : {...emptyForm, branchId: new URLSearchParams((window.location.hash.split('?')[1] || '')) .get('branchId') || ''});
  };
  ...
  return <section className="space-y-5">
    ...
    {editing&&<div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/70 p-4"><form onSubmit={submit} className="w-full max-w-lg rounded-2xl bg-white p-6"><div className="mb-5 flex justify-between"><div><h2 className="text-xl font-black">{editing.id?'Edit Staff':'Create Staff Account'}</h2><p className="text-sm text-gray-500">Assign branch and issue a temporary staff PIN.</p></div><button type="button" onClick={()=>setEditing(null)}><X/></button></div><label className="block text-sm font-bold">Staff Name *<input required value={form.name} onChange={event=>setForm({...form,name:event.target.value})} className="mt-1 w-full rounded-xl border p-3"/></label>{!editing.id&&<label className="mt-4 block text-sm font-bold">Email<input required type="email" value={form.email} onChange={event=>setForm({...form,email:event.target.value})} className="mt-1 w-full rounded-xl border p-3"/></label>}<label className="mt-4 block text-sm font-bold">Staff Code *<input required value={form.username} onChange={event=>setForm({...form,username:event.target.value.toLowerCase()})} className="mt-1 w-full rounded-xl border p-3"/></label><label className="mt-4 block text-sm font-bold">Phone<input value={form.phone} onChange={event=>setForm({...form,phone:event.target.value})} className="mt-1 w-full rounded-xl border p-3"/></label><label className="mt-4 block text-sm font-bold">Role *<select value={form.role} onChange={event=>setForm({...form,role:event.target.value})} className="mt-1 w-full rounded-xl border p-3">{roles.map(role=><option key={role}>{role}</option>)}</select></label><label className="mt-4 block text-sm font-bold">Primary Branch *<select required value={form.branchId} onChange={event=>setForm({...form,branchId:event.target.value})} className="mt-1 w-full rounded-xl border p-3"><option value="">Select branch</option>{branches.filter(branch=>branch.status==='ACTIVE').map(branch=><option key={branch.id} value={branch.id}>{branch.code} · {branch.name}</option>)}</select></label><p className="mt-3 rounded-xl bg-slate-50 p-3 text-xs text-slate-600">A Staff record is created once. Branch assignments determine where the Staff can work.</p><label className="mt-4 flex items-center gap-3 rounded-xl border p-3 text-sm font-bold"><input type="checkbox" checked={form.enablePosAccess} onChange={event=>setForm({...form,enablePosAccess:event.target.checked})}/>Enable POS Access<span className="ml-auto text-xs font-normal text-gray-400">Generate temporary PIN</span></label>{editing.id&&<label className="mt-4 block text-sm font-bold">Status<select value={form.status} onChange={event=>setForm({...form,status:event.target.value})} className="mt-1 w-full rounded-xl border p-3"><option>ACTIVE</option><option>INACTIVE</option><option>LOCKED</option></select></label>}<div className="mt-6 flex gap-3"><button type="button" onClick={()=>setEditing(null)} className="flex-1 rounded-xl border p-3 font-bold">Cancel</button><button disabled={state.busy} className="flex-1 rounded-xl bg-[#D4AF37] p-3 font-black">{state.busy?'Saving…':'Create Staff'}</button></div></form></div>}

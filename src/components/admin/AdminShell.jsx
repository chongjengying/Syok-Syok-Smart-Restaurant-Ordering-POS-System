...
import {Activity,ArrowLeft,ChartNoAxesCombined,ClipboardList,FolderTree,LayoutDashboard,Logs,Menu,PackageSearch,ReceiptText,Settings,ShieldCheck,TrendingUp,Users,UtensilsCrossed,X,QrCode} from 'lucide-react';
...
const groups=[
  ...
  ['SYSTEM',[['system-administration','System Administration','settings.view',Settings],['system-health','System Health','system.health.view',Activity],['audit','Audit Logs','audit.view',Logs],['qr-settings','DuitNow QR','settings.manage',QrCode]]],
];
...
export default function AdminShell({role,permissions,onBack,lang}){
  ...
  const navigate=(nextSection,filters={})=>{
    if(!globalThis.dispatchEvent(new CustomEvent('before-admin-navigate',{cancelable:true,detail:{section:nextSection}})))return;
  ...
  const content={
    dashboard:<AdminDashboard onNavigate={navigate} systemHealth={healthState.data} canViewSystemAdministration={allowed.has('settings.view')}/>,
    ...
    'system-administration':<SystemAdministrationPage/>,
  }[section];
  ...
  return <div className="flex h-full bg-[#F5F6F8] text-[#121212]"><div className="hidden shrink-0 lg:block">{sidebar}</div>{sidebarOpen&&<div className="fixed inset-0 z-[80] lg:hidden"><button className="absolute inset-0 bg-black/60" onClick={()=>setSidebarOpen(false)} aria-label="Close navigation overlay"/><div className="relative h-full">{sidebar}</div></div>}<main className="min-w-0 flex-1 overflow-y-auto"><div className="sticky top-0 z-30 flex items-center justify-between gap-3 border-b bg-white/95 p-3 backdrop-blur"><button onClick={()=>setSidebarOpen(true)} className="flex items-center gap-2 rounded-lg border px-3 py-2 text-sm font-bold lg:hidden"><Menu size={18}/>Admin menu</button><span className="hidden lg:block"/>{allowed.has('system.health.view')&&<OperationalIndicator health={healthState.data} onClick={()=>navigate('system-health')}/>}</div><div key={routeKey} className="p-4 sm:p-6">{content}</div></main></div>;
}

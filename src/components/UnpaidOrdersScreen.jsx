...
export default function UnpaidOrdersScreen({ onBack, onOpenOrder }) {
    ...
    <div className="flex h-full w-full flex-col overflow-hidden bg-[#F5F6F8] text-[#121212]">
      <header className="flex h-16 shrink-0 items-center justify-between bg-[#121212] px-6 text-white">
        ...
        <div className="flex items-center gap-2 font-black uppercase tracking-wider">
          <ReceiptText className="h-5 w-5 text-[#D4AF37]" /> Unpaid Orders
        </div>
      ...
      <main className="flex-1 overflow-y-auto p-6">
        ...
        {isLoading ? (
          <div className="flex h-full items-center justify-center gap-3 text-gray-500"><Loader2 className="h-6 w-6 animate-spin" /> Loading unpaid orders…</div>
        ) : orders.length === 0 ? (
          <div className="flex h-full flex-col items-center justify-center text-gray-400">
            <ReceiptText className="mb-3 h-12 w-12" />
            <p className="font-bold text-gray-600">No unpaid orders</p>
          </div>
        ) : (
              ...
              <article key={order.id} className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
                <div className="flex items-start justify-between gap-3 border-b border-gray-100 pb-3">
                  ...
                  <div className="text-right">
                    <span className="rounded-full bg-amber-100 px-3 py-1 text-[10px] font-black text-amber-800">UNPAID</span>
                    <p className="mt-2 text-xl font-black text-[#B8952B]">${order.total.toFixed(2)}</p>
                  </div>
                </div>
                  ...
                  {order.items.map((item) => (
                    <div key={item.id} className="flex justify-between gap-3 text-sm">
                      <div>
                        ...
                        {item.serviceMode === 'TAKEAWAY' && <span className="ml-2 text-[10px] font-black text-sky-700">🥡 TAKEAWAY</span>}
                        <span className="ml-2 text-[9px] font-black text-[#9A7618]">{item.batchNo > 1 ? `ADD-ON • ROUND ${item.batchNo}` : 'ROUND 1'}</span>
                      </div>
                ...
                <div className="flex items-center justify-between border-t border-gray-100 pt-3">
                  ...
                  <button onClick={() => void openOrder(order)} className="rounded-xl bg-[#121212] px-4 py-2 text-xs font-bold text-[#D4AF37]">View Order</button>
                </div>
              </article>
            ))}
          </div>

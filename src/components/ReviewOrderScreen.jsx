...
export default function ReviewOrderScreen({
  ...
  const notedItems = cart.filter((item) => item.specialRequest?.trim());
    ...
    <div className="flex h-full w-full flex-col overflow-hidden bg-[#F5F6F8] text-[#121212]">
      <header className="flex h-16 shrink-0 items-center justify-between bg-[#121212] px-6 text-white">
        <button onClick={onEdit} disabled={isSending} className="flex items-center gap-2 text-sm font-bold text-gray-300 hover:text-[#D4AF37] disabled:opacity-50">
          <ArrowLeft className="h-5 w-5" /> {tr('editOrder')}
        </button>
        <h1 className="font-black uppercase tracking-widest">{tr('reviewOrder')}</h1>
        <div className="w-20" />
      </header>
        ...
        <section className="w-full max-w-xl overflow-hidden rounded-3xl border border-gray-200 bg-white shadow-xl">
          <div className="flex items-center justify-between bg-[#121212] px-6 py-5 text-white">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-[10px] font-black uppercase tracking-wider text-gray-400">{tr('orderDestination')}</p>
                <p className="font-black">{diningMode === 'dine-in' ? `${tr('table')}: ${selectedTable || '-'}` : tr('takeaway')}</p>
              </div>
            </div>
            {isAddOn && <span className="rounded-full bg-amber-400/15 px-3 py-1 text-[10px] font-black text-amber-300">{tr('addOnRoundLabel')}</span>}
          </div>
          ...
          <div className="p-6">
              ...
              {cart.map((item, index) => (
                <div key={`${item.dish.id}-${index}`} className="border-b border-gray-100 pb-3">
                  <div className="flex items-start justify-between gap-4">
                    <div>
                      <p className="font-black"><span className="text-gray-500">{item.quantity} ×</span> {item.dish.name}</p>
            ...
            <div className="mt-5 rounded-2xl border border-amber-200 bg-amber-50 p-4">
              <p className="text-[10px] font-black uppercase tracking-wider text-amber-800">{tr('specialNote')}</p>
              {notedItems.length > 0 ? notedItems.map((item, index) => (
                <p key={`${item.dish.id}-${index}`} className="mt-1 text-sm font-semibold text-amber-950">
                  {notedItems.length > 1 ? `${item.dish.name}: ` : ''}{item.specialRequest}
                </p>
              )) : <p className="mt-1 text-sm text-amber-700">{tr('noSpecialNote')}</p>}
            </div>

            <div className="mt-5 flex justify-between border-y border-gray-300 py-4 text-xl font-black">
              <span>{tr('total')}</span><span>{money(previewTotals.total)}</span>
            </div>

            {diningMode === 'takeaway' && (
              <section className="mt-4 rounded-2xl border border-sky-200 bg-sky-50 p-4">
                <h2 className="text-xs font-black uppercase tracking-wider text-sky-900">{tr('takeawayPackaging')}</h2>
                <p className="mt-1 text-[10px] text-sky-700">{tr('packagingHelp')}</p>
                <div className="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-3">
                  {packagingOptions.map((value) => {
            ...
            <p className="mt-3 text-center text-[10px] text-gray-500">{tr('authoritativeNotice')}</p>
            ...
            <div className="mt-5 grid grid-cols-2 gap-3">
              <button disabled={isSending} onClick={onEdit} className="rounded-xl border border-gray-300 bg-white px-4 py-4 font-black disabled:opacity-50">← {tr('editOrder')}</button>
              <button
              ...
              >
                {isSending ? <><Loader2 className="h-5 w-5 animate-spin" /> {tr('processing')}</> : <>{diningMode === 'takeaway' ? tr('continuePay') : tr('sendOrder')} <Send className="h-5 w-5" /></>}
              </button>
            </div>
            <div className="mt-4 flex items-center justify-center gap-2 text-xs font-semibold text-emerald-700"><CheckCircle2 className="h-4 w-4" /> {diningMode === 'takeaway' ? tr('takeawayPaymentNotice') : tr('dineInSubmitNotice')}</div>
          </div>
        </section>
      </main>
    </div>

...
export default function OrderDetailScreen({ orderId, canPay, onBack, onAddItems, onPayment }) {
    ...
    <div className="flex h-full w-full flex-col overflow-hidden bg-[#F5F6F8] text-[#121212]">
      <header className="flex h-16 shrink-0 items-center justify-between bg-[#121212] px-6 text-white">
        <button onClick={onBack} className="flex items-center gap-2 text-sm font-bold text-gray-300 hover:text-[#D4AF37]">
          <ArrowLeft className="h-5 w-5" /> Orders
        </button>
        <div className="flex items-center gap-2 font-black uppercase tracking-wider">
          <ReceiptText className="h-5 w-5 text-[#D4AF37]" /> Order Detail
        </div>
        <div className="w-20" />
      </header>
        ...
        <div className="mx-auto max-w-3xl overflow-hidden rounded-3xl border border-gray-200 bg-white shadow-xl">
          {isLoading ? (
            <div className="flex min-h-96 items-center justify-center gap-3 text-gray-500">
              <Loader2 className="h-6 w-6 animate-spin" /> Loading active order from Supabase…
            </div>
          ) : error || !order ? (
            <div className="m-6 flex gap-3 rounded-xl border border-red-200 bg-red-50 p-4 text-sm text-red-700">
              <AlertTriangle className="h-5 w-5 shrink-0" /> {error || 'Order was not found.'}
            </div>
          ) : (
            <>
              <section className="flex flex-wrap items-start justify-between gap-4 bg-[#121212] p-6 text-white">
                <div>
                  <p className="flex items-center gap-2 text-sm font-bold text-[#D4AF37]">
                    ...
                    {order.diningMode === 'dine-in' ? `Table ${order.table?.tableNumber || '-'}` : 'Takeaway'}
                  </p>
                ...
                <div className="text-right">
                  <span className="rounded-full bg-white/10 px-3 py-1 text-[10px] font-black text-[#D4AF37]">{order.status}</span>
                  <p className="mt-3 text-xs font-bold text-amber-300">{order.paymentStatus}</p>
                </div>
              </section>

              <section className="p-6">
                <h2 className="mb-3 text-xs font-black uppercase tracking-wider text-gray-500">Order Rounds</h2>
                  ...
                  {groupOrderRounds(order.items).map((round) => (
                    <section key={round.roundNo} className="rounded-2xl border border-gray-200 p-4">
                      <div className="mb-3 flex items-center justify-between border-b border-gray-100 pb-2">
                        <span className="text-[10px] font-black text-amber-800">{round.isAddOn ? `ADD-ON • ROUND ${round.roundNo}` : `ROUND ${round.roundNo}`}</span>
                        <span className="rounded-full bg-gray-100 px-2 py-0.5 text-[9px] font-black text-gray-600">{round.status}</span>
                      </div>
                ...
                <div className="ml-auto mt-5 max-w-sm space-y-2 text-sm">
                  ...
                  <div className="flex justify-between"><span className="text-gray-500">Discount</span><span>- {money(order.discount)}</span></div>
                  <div className="flex justify-between"><span className="text-gray-500">Tax</span><span>{money(order.tax)}</span></div>
                  {order.serviceCharge > 0 && (
                    <div className="flex justify-between"><span className="text-gray-500">Service charge</span><span>{money(order.serviceCharge)}</span></div>
                  )}
              ...
              <footer className="border-t border-gray-200 bg-gray-50 p-5">
                {!paymentReady && order.paymentStatus === 'UNPAID' && (
                  <p className="mb-3 text-center text-xs font-semibold text-amber-700">
                    {hasUnsentItems
                      ? 'Send or remove all draft items before payment.'
                ...
                <div className="flex flex-col gap-3 sm:flex-row sm:justify-end">
                  ...
                  {canPay && (
                    <button
                      ...
                      onClick={onPayment}
                      className="flex items-center justify-center gap-2 rounded-xl bg-emerald-500 px-6 py-3 text-sm font-black text-black disabled:cursor-not-allowed disabled:bg-gray-300 disabled:text-gray-500"
                    >
                      <CreditCard className="h-5 w-5" /> View Bill / Pay
                    </button>
                  )}
                </div>
              </footer>

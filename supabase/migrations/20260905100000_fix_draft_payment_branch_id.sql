-- Draft payments must carry the same branch as their order.
...
declare uid uuid := auth.uid(); key text; existing public.orders%rowtype;
...
begin
  ...
  select coalesce((select branch_id from public.restaurant_tables where id=p_table_id),(select branch_id from public.profiles where id=uid),(select id from public.branches where code='MAIN')) into draft_branch_id;
  ...
  insert into public.payments(order_id,user_id,branch_id,payment_method,amount,reference,status,paid_at)
  values(new_order.id,uid,draft_branch_id,'CASH',0,number_value,'PENDING',null) returning * into new_payment;

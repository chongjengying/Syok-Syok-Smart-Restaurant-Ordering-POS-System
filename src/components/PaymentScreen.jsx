...
export default function PaymentScreen({ orderId, onBack, onPaymentSubmit }) {
  ...
  return (
    <div className="flex h-full w-full flex-col overflow-hidden bg-[#F5F6F8] text-[#121212]">
      <header className="flex h-16 shrink-0 items-center justify-between bg-[#121212] px-6 text-white">
        ...
        <h1 className="font-black uppercase tracking-wider">Payment</h1>
      ...
      <main className="flex-1 overflow-y-auto p-4 sm:p-7">
        <div className="mx-auto grid max-w-6xl gap-6 lg:grid-cols-[1.2fr_0.8fr]">
          <section className="overflow-hidden rounded-3xl border border-gray-200 bg-white shadow-sm">
            {isLoadingOrder ? (
              <div className="flex min-h-96 items-center justify-center gap-3 text-gray-500"><Loader2 className="h-6 w-6 animate-spin" /> Loading authoritative order…</div>
            ) : orderError || !order ? (
              <div className="m-6 flex gap-2 rounded-xl border border-red-200 bg-red-50 p-4 text-sm text-red-700"><AlertTriangle className="h-5 w-5" /> {orderError || 'Order was not found.'}</div>
            ) : (
              <>
                <div className="flex justify-between bg-[#121212] p-5 text-white">
                  <div>
                    <p className="text-xs font-bold text-[#D4AF37]">{order.diningMode === 'dine-in' ? `Table ${order.table?.tableNumber || '-'}` : 'Takeaway'}</p>
                    <h2 className="mt-1 text-xl font-black">{order.orderNumber}</h2>
                  </div>
                  <div className="text-right text-xs"><p>{order.status}</p><p className="mt-1 text-amber-300">{order.paymentStatus}</p></div>
                </div>
                <div className="p-5">
                  {order.diningMode === 'takeaway' && order.takeawayPackaging.length > 0 && (
                    <div className="mb-4 rounded-xl border border-sky-200 bg-sky-50 p-3">
                      <p className="text-[10px] font-black uppercase text-sky-800">Packaging</p>
                      <p className="mt-1 text-xs font-semibold text-sky-900">{order.takeawayPackaging.map((entry) => entry.replaceAll('_', ' ')).join(' · ')}</p>
                    </div>
                  )}
                  <div className="max-h-72 space-y-4 overflow-y-auto">
                    {groupOrderRounds(orderItems).map((round) => (
                      <section key={round.roundNo}>
                        <p className="mb-2 text-[10px] font-black text-amber-800">{round.isAddOn ? `ADD-ON • ROUND ${round.roundNo}` : `ROUND ${round.roundNo}`}</p>
                  ...
                  <div className="ml-auto mt-5 max-w-sm space-y-2 text-sm">
                    ...
                    {order.serviceCharge > 0 && <div className="flex justify-between"><span>Service charge</span><span>{money(order.serviceCharge)}</span></div>}
                    <div className="flex justify-between border-t-2 border-[#121212] pt-3 text-xl font-black"><span>Total</span><span>{money(order.total)}</span></div>
                  </div>
                </div>
              </>
            )}
          ...
          <section className="rounded-3xl border border-gray-200 bg-white p-5 shadow-sm">
            <h2 className="font-black">Choose payment method</h2>
            {isLoadingMethods ? (
              <div className="flex h-32 items-center justify-center gap-2 text-sm text-gray-500"><Loader2 className="h-5 w-5 animate-spin" /> Loading methods…</div>
            ) : (
                ...
                {capabilities.map((capability) => {
                  ...
                  return (
                    <button key={capability.method} disabled={!capability.available || isProcessing} onClick={() => { setSelectedMethod(capability.method); setPaymentError(''); }} className={`rounded-xl border-2 p-4 text-left ${selected ? 'border-[#D4AF37] bg-[#121212] text-white' : 'border-gray-200 bg-white'} disabled:opacity-40`}>
                      ...
                      <p className="mt-3 text-sm font-black">{details.label}</p>
                    </button>
                  );
                })}
              </div>
            ...
            {selectedMethod === 'CASH' && (
              <div className="mt-5 rounded-2xl bg-gray-50 p-4">
                <label htmlFor="received-amount" className="text-xs font-black uppercase text-gray-500">Cash received</label>
                ...
                <div className="mt-3 flex justify-between text-sm"><span>Change</span><strong className="text-emerald-700">{money(changeAmount)}</strong></div>
                {receivedInput && !validCashReceived && <p className="mt-2 text-xs font-semibold text-red-600">Received amount must cover the total.</p>}
              </div>
            )}

            {!isLoadingOrder && order && !orderPayable && <div className="mt-4 rounded-xl border border-amber-200 bg-amber-50 p-3 text-xs font-semibold text-amber-800">{hasUnsentItems ? 'Dine-in draft items must be submitted before payment.' : `This order cannot be paid in its current state: ${order.status} / ${order.paymentStatus}.`}</div>}
            {!isLoadingOrder && orderPayable && hasActiveKitchenItems && (
              <label className="mt-4 flex cursor-pointer items-start gap-3 rounded-xl border border-amber-300 bg-amber-50 p-3 text-xs font-semibold text-amber-900">
                ...
                />
                <span>
                  Kitchen rounds are still waiting, preparing, or ready to serve. I confirm this final payment completes only this bill. Kitchen fulfillment continues and the occupied table may start a separate new bill.
                </span>
              </label>
            )}
            {paymentError && <div className="mt-4 rounded-xl border border-red-200 bg-red-50 p-3 text-xs font-semibold text-red-700">{paymentError}{capabilitiesError && <button onClick={() => refresh()} className="ml-2 underline">Retry</button>}</div>}
            ...
            <button disabled={!canConfirm} onClick={() => void handlePayment()} className="mt-5 w-full rounded-xl bg-emerald-500 px-5 py-4 font-black text-black disabled:cursor-not-allowed disabled:bg-gray-300 disabled:text-gray-500">
              {isProcessing ? 'Recording payment…' : takeawayAwaitingPayment ? 'Pay & Submit Takeaway' : 'Confirm Payment'}
            </button>
            <p className="mt-3 text-center text-[10px] text-gray-500">The database validates the final amount inside complete_payment().</p>
          </section>
        </div>
      </main>
    </div>
